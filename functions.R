...
#take a named vector and convert to a data frame with the names turned into 
#a column
...
#take an input, and calculate the cumulative counts of rows sorted by the 
#target column
cumulativeCounts <- function(dists, smooth = T, target_column = 'max_dist'){
  ...
  for(i in groups){
    ...
    dat <- dat %>% mutate(count = 1) %>%
      ...
      group_by(group, .data[[target_column]]) %>% 
      summarise(cumulative_prop = max(cumulativeCount)/ nrow(dat))
    if(smooth){
      dat <- as.data.frame(spline(x = dat[[target_column]],
                                  y =  dat$cumulative_prop))
    }
...
cumulativeDistribution <- function(dat, run.ks.test = F, 
                                   alternative_pred, 
                                   alternative_pc, 
...
printListSubset <- function(inList, vec = NA, 
                            startText = NA, 
                            getNames = F, 
                            round_val = 10){
  if(getNames){
  ...
  for(value in vec){
      if(value %in% names(inList)){
        outputPrint = paste0(outputPrint, 
                             value, 
                             ': ', 
                             round(inList[[value]], 
                                   round_val), '\n  ')
...
#obtain a list of ids of overlapping alignments
...
#take the distances matrix and calculate the maximum distance between any pair
#of sequences within each alignment
calculateMaximumDistances <- function(group){
  pairs <- read.table(paste0(group, "/query_target_pairs.txt"))
  colnames(pairs) <- c("target.name", "query.genome", "ID")
  contig_labels <- read.table("genome_contig_pairs.txt") #reloading this for quick
  #restart of troubleshooting.
  colnames(contig_labels) <- c("target.name", "target.genome")
  #add the genome ids to the contig ids of each of the sequences in the alignments
  pairs <- pairs %>% left_join(contig_labels, by = 'target.name')
  pairs <- pairs %>% select(target.genome, ID) %>% unique()
  
  #get a list of the alignment IDs
  srnas <- unique(pairs$ID)
  #setup a dataframe to store the max distances for each alignment
  max_dists <- data.frame(id = srnas, distance = NA) 
  counter <- 0
  
  #for each alignment get the maximum distance between any pair of sequences
  for(item in 1:nrow(max_dists)){
    
    #set the current max distance to zero 
    max_val <- 0
    
    #select all the sequences in the item alignment
    df <- pairs %>% filter(ID == as.character(max_dists$id[item])) %>% unique()
    
    #check if there was only a single sequence
    if(nrow(df) == 1){
      max_val <- 0
    ...
    #get list of genomes for sequences in alignment 
    genomes <- df$target.genome
    
    #Select the rows and columns that correspond to the genomes and
    #fetch the maximum distance from the subset
...
calculateCorrelationMatrix <- function(dat, matNames){
  randomNum <- runif(n = nrow(dat), min = 0, max = 1)
  dat$random <- randomNum
  
  corInput <- dat %>% 
    mutate(`Known sRNA or RINC` = ifelse(group == "Positive Control", 
                                         1, 
                                         0)) %>% 
    select(-group)
  corInput <- corInput %>% filter(distance < 0.15)
  colnames(corInput) <- matNames 
  ...
  for(i in 1:length(dNames)){
    for(j in 1:length(dNames)){
      spear<-cor.test(corInput[,dNames[i] == colnames(corInput)], 
                      corInput[,dNames[j] == colnames(corInput)], 
  ...
  return(list(rhoMatrix=rhoMatrix, sigMatrix=sigMatrix))
}
#